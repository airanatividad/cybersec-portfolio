...
function createImageGlitch(element) {
    const randomGlitch = () => {
        if (Math.random() < 0.1) { // 10% chance to trigger glitch
            element.style.transform = `translate(${Math.random() * 10 - 5}px, ${Math.random() * 10 - 5}px)`;
            element.style.filter = `hue-rotate(${Math.random() * 360}deg) saturate(${Math.random() * 2 + 0.5})`;
...
function toggleTheme() {
    ...
    // Update glitch effects for new theme
    updateGlitchEffects(newTheme);
}
